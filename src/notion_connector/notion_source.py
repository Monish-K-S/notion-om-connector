...
# OpenMetadata Base Classes
from metadata.ingestion.api.steps import Source
from metadata.ingestion.api.models import Entity

# OpenMetadata Hierarchy & Table Schemas
...
# OpenMetadata Type Mapping & Helpers
...
class NotionSource(Source):
    ...
    def _iter(self) -> Iterable[Entity]:
        # 1. REGISTER SERVICE (Hierarchy)
        yield CreateDatabaseServiceRequest(
            name=self.service_name,
            serviceType=DatabaseServiceType.CustomDatabase,
            description="## 🗝️ Notion Enterprise Workspace\n**Owner:** Data Governance Team\n\nAutomated metadata reconciliation for Notion databases.",
            connection=DatabaseConnection(config=CustomDatabaseConnection(type="CustomDatabase", connectionOptions={}))
        )
        ...
        for obj in response:
            if obj.get("object") in ["database", "data_source"]:
                ...
                # Keep the Rich Description (This doesn't require server-side tags)
                description = f"### 📋 Notion Source Documentation\n- **Notion ID:** `{obj_id}`\n- **Source Link:** [View in Notion](https://www.notion.so/{obj_id})\n"

                yield CreateTableRequest(
                    name=fqn.split(".")[-1],
                    description=description,
                    columns=[map_notion_property_to_column(n, d) for n, d in props.items()],
                    databaseSchema=f"{self.service_name}.Notion_Workspace.default"
                    # TAGS REMOVED TO PREVENT VALIDATION ERROR
                )
        ...
        for rel in self.relationships:
            if rel["to_id"] not in self.id_to_fqn:
                try:
                    ...
                    res_name = remote_db.get("title", [{"plain_text":"Rescued"}])[0].get("plain_text").replace(" ", "_")
